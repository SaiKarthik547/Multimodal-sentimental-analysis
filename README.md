...
Input: The user provides a social media URL in the "Analysis Input" tab.

Data Fetching: The fetch_social_media_data function identifies the platform.
...
Component Analysis: The app analyzes each component it successfully fetched.
...
BLIP (VLM) generates a caption describing the scene.

This text (OCR + Caption) is analyzed by the text ensemble.

detect_emotions_ensemble (fer + deepface) finds all faces and averages their emotion scores.

The final image score is a weighted fusion of the text-derived sentiment and the emotion-derived sentiment.
...
Whisper transcribes the audio to text, which is then analyzed by the text ensemble.

Wav2Vec2 analyzes the audio for emotional tone.

The scores from the transcript sentiment and the audio emotion are averaged.

Sentiment Fusion: The fuse_sentiments_adaptive function receives the individual sentiment scores (from 0.0 to 1.0) for Text, Media, and Comments.
...
git clone [https://github.com/your-username/social-media-sentiment-analyzer.git](https://github.com/your-username/social-media-sentiment-analyzer.git)
...
Paste a supported social media URL into the text box on the "Analysis Input" tab.

Click "Analyze Now ✨".

Wait for the analysis to complete (this can take 1-2 minutes for videos).

View the detailed results, metrics, and plots on the "Analysis Results" tab.

View past runs on the "History" tab.
